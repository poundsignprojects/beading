...
test('applyMove: shifts moving cells by the delta and clears their old position', () => {
  ...
  const patch = applyMove(cells, baseCells, [[0, 0]], (row, col) => ({ row: row + 2, col: col + 3 }), 20, 20, new Set());
...
test('applyMove: moving content overwrites whatever is already at the destination (front semantics)', () => {
  ...
  const cells = new Map(baseCells);
  const patch = applyMove(cells, baseCells, [[0, 0]], (row, col) => ({ row: row + 0, col: col + 1 }), 20, 20, new Set());
...
test('applyMove: a destination outside the grid is dropped (clipped, not shifted), matching applyPaste', () => {
  ...
  const cells = new Map(baseCells);
  const patch = applyMove(cells, baseCells, [[4, 4]], (row, col) => ({ row: row + 0, col: col + 1 }), 5, 5, new Set()); // col 5 is out of bounds (0-4 valid)
...
test('applyMove: recomputing at a new delta against the same baseCells does not drift from a previous call\'s leftovers', () => {
  ...
  applyMove(cells, baseCells, [[0, 0]], (row, col) => ({ row: row + 5, col: col + 5 }), 20, 20, touchedKeys); // simulate an earlier drag frame
  const patch = applyMove(cells, baseCells, [[0, 0]], (row, col) => ({ row: row + 1, col: col + 1 }), 20, 20, touchedKeys); // then a later frame, smaller delta
...
test('applyMove: a delta of (0,0) is a full no-op with an empty patch', () => {
  ...
  setCell(baseCells, 0, 0, 'red');
  const cells = new Map(baseCells);
  const patch = applyMove(cells, baseCells, [[0, 0]], (row, col) => ({ row: row + 0, col: col + 0 }), 20, 20, new Set());
...
test('applyMove: content outside the moving set is left completely untouched', () => {
  ...
  const cells = new Map(baseCells);
  const patch = applyMove(cells, baseCells, [[0, 0]], (row, col) => ({ row: row + 1, col: col + 1 }), 20, 20, new Set());
  assert.equal(cells.get('9,9').colorId, 'green');
  assert.equal(patch.some((entry) => entry.row === 9 && entry.col === 9), false);
});

test('applyMove: computeTarget can shift different cells by different amounts (per-cell compensation, not just a uniform delta)', () => {
  const baseCells = new Map();
  setCell(baseCells, 5, 5, 'a');
  setCell(baseCells, 5, 6, 'b'); // this one gets an extra +1 row from computeTarget, the other doesn't
  const cells = new Map(baseCells);
  const computeTarget = (row, col) => ({ row: row + (col === 6 ? 1 : 0), col: col + 1 });
  const patch = applyMove(cells, baseCells, [[5, 5], [5, 6]], computeTarget, 20, 20, new Set());
  assert.equal(cells.get('5,6').colorId, 'a'); // col 5 -> col 6, row unchanged
  assert.equal(cells.get('6,7').colorId, 'b'); // col 6 -> col 7, row +1
  assert.equal(cells.has('5,5'), false);
  assert.equal(cells.has('5,7'), false); // not where a uniform (row+0,col+1) delta would have put it
  assert.equal(cells.size, 2);
  // "5,6" is a single touched key carrying both effects at once: b's original
  // color there (before) is overwritten by a's moved-in color (after).
  assert.deepEqual(patch.sort((a, b) => a.col - b.col), [
    { row: 5, col: 5, before: { colorId: 'a' }, after: undefined },
    { row: 5, col: 6, before: { colorId: 'b' }, after: { colorId: 'a' } },
    { row: 6, col: 7, before: undefined, after: { colorId: 'b' } },
...
test('applyMove: moving a whole blob by a small delta over itself does not clobber content read from stale cells', () => {
  ...
  applyMove(cells, baseCells, moving, (row, col) => ({ row: row + 0, col: col + 1 }), 20, 20, new Set());
